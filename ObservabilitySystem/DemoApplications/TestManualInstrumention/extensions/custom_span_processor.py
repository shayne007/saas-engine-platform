        ...
        try:
            ...
            try:
                if self._should_process_span(span):
                    ...
                    start_dt = datetime.datetime.utcfromtimestamp(span.start_time / 1e9)
                    end_dt = datetime.datetime.utcfromtimestamp(span.end_time / 1e9)
                    logger.info(f"Span start time: {start_dt.isoformat()}Z")
                    logger.info(f"Span end time: {end_dt.isoformat()}Z")
                    logger.info(f"Span data as json: {json.dumps(span_dict)}")
