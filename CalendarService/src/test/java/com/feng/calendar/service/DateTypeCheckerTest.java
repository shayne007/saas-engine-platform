...
class DateTypeCheckerTest {
    ...
    @BeforeEach
    void setUp() {
        dateTypeChecker = new DateTypeChecker(holidayResolver, weekendChecker, businessCalendarService);
    }
    ...
    void testCheckDateType_WorkDay() {
        ...
        // When
        DateTypeResponse response = dateTypeChecker.checkDateType(date, countryCode, null);
    ...
    void testCheckDateType_Holiday() {
        ...
        when(weekendChecker.isWeekend(date, countryCode)).thenReturn(false);
        when(holidayResolver.findHoliday(date, countryCode)).thenReturn(Optional.of(holiday));
    ...
    void testCheckDateType_CustomNonWorkDay() {
        ...
        when(weekendChecker.isWeekend(date, countryCode)).thenReturn(false);
        when(holidayResolver.findHoliday(date, countryCode)).thenReturn(Optional.empty());
        when(businessCalendarService.isBusinessCalendarActive(businessCalendarId)).thenReturn(true);
        when(businessCalendarService.getCustomRule(date, businessCalendarId)).thenReturn(Optional.of(rule));
        
        // When
        DateTypeResponse response = dateTypeChecker.checkDateType(date, countryCode, businessCalendarId);
