 ...
 */
@Repository
public interface BusinessCalendarRuleRepository extends JpaRepository<BusinessCalendarRule, Long> {
    
    /**
     * Find rule for specific date and business calendar
     */
    @Query("SELECT bcr FROM BusinessCalendarRule bcr " +
           "WHERE bcr.businessCalendar.id = :calendarId AND bcr.date = :date AND bcr.isActive = true")
    Optional<BusinessCalendarRule> findByCalendarIdAndDate(@Param("calendarId") Long calendarId,
    ...
    @Query("SELECT bcr FROM BusinessCalendarRule bcr " +
           ...
           "ORDER BY bcr.date")
    List<BusinessCalendarRule> findByCalendarIdAndDateRange(@Param("calendarId") Long calendarId,
     ...
     */
    @Query("SELECT bcr FROM BusinessCalendarRule bcr " +
           "WHERE bcr.businessCalendar.id = :calendarId AND bcr.ruleType = :ruleType AND bcr.isActive = true")
    List<BusinessCalendarRule> findByCalendarIdAndRuleType(@Param("calendarId") Long calendarId,
    ...
    @Query("SELECT bcr FROM BusinessCalendarRule bcr " +
           ...
           "ORDER BY bcr.date")
    List<BusinessCalendarRule> findActiveByCalendarId(@Param("calendarId") Long calendarId);
    ...
    List<BusinessCalendarRule> findRecurringByCalendarId(@Param("calendarId") Long calendarId);
}
