...
public interface HolidayRepository extends JpaRepository<Holiday, Long> {
	 ...
	 */
	@Query("SELECT h FROM Holiday h JOIN h.country c WHERE c.code = :countryCode AND h" +
			".date = :date")
	 ...
	 */
	@Query("SELECT h FROM Holiday h JOIN h.country c WHERE c.code = :countryCode ORDER " +
			"BY h.date")
	...
	List<Holiday> findRecurringHolidaysByCountryCode(
