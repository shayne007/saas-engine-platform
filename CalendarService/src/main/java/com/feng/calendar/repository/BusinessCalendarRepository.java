...
public interface BusinessCalendarRepository
		extends JpaRepository<BusinessCalendar, Long> {

	/**
	 * Find business calendar by organization ID and country
	 */
	@Query("SELECT bc FROM BusinessCalendar bc JOIN bc.country c " +
			"WHERE bc.organizationId = :organizationId AND c.code = :countryCode AND bc" +
			".isActive = true")
	Optional<BusinessCalendar> findByOrganizationIdAndCountryCode(
	 ...
	 */
	@Query("SELECT bc FROM BusinessCalendar bc LEFT JOIN FETCH bc.rules WHERE bc.id = " +
			":id AND bc.isActive = true")
	...
	List<BusinessCalendar> findActiveByCountryCode(
			@Param("countryCode") String countryCode);

	/**
	 * Find business calendar by name and organization
	 */
	Optional<BusinessCalendar> findByNameAndOrganizationId(String name,
