...
public interface FileRepository extends JpaRepository<FileEntity, UUID> {
    ...
    List<FileEntity> findByProjectIdAndUploadStatus(UUID projectId, FileEntity.UploadStatus status);
    
    List<FileEntity> findByCreatedByAndUploadStatus(UUID createdBy, FileEntity.UploadStatus status);
    
    @Query(value = "SELECT f.*, " +
            "COUNT(*) OVER() as total_count " +
            "FROM files f " +
            "WHERE (:projectId IS NULL OR f.project_id = :projectId) " +
            "AND (:mimeTypes IS NULL OR f.mime_type = ANY(:mimeTypes)) " +
    ...
    @Query("SELECT COUNT(f) FROM FileEntity f WHERE f.projectId = :projectId AND f.uploadStatus = 'COMPLETED'")
    long countByProjectIdAndStatus(@Param("projectId") UUID projectId);
    
    @Query("SELECT SUM(f.fileSize) FROM FileEntity f WHERE f.projectId = :projectId AND f.uploadStatus = 'COMPLETED'")
