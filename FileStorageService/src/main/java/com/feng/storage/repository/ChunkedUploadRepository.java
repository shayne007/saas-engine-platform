...
@Repository
public interface ChunkedUploadRepository extends JpaRepository<ChunkedUploadEntity, UUID> {
    
    Optional<ChunkedUploadEntity> findByUploadId(String uploadId);
    
    List<ChunkedUploadEntity> findByFileId(UUID fileId);
    
    @Query("SELECT COUNT(c) FROM ChunkedUploadEntity c WHERE c.file.uploadStatus IN :statuses")
    ...
    @Query("SELECT c FROM ChunkedUploadEntity c WHERE c.fileId = :fileId AND c.completedChunks = c.totalChunks")
    Optional<ChunkedUploadEntity> findCompletedUploadByFileId(@Param("fileId") UUID fileId);
}
