...
@Repository
public interface FileAccessLogRepository
		extends JpaRepository<FileAccessLogEntity, UUID> {
	...
	@Query("SELECT l FROM FileAccessLogEntity l WHERE l.fileId = :fileId AND l" +
			".accessType = :accessType ORDER BY l.accessedAt DESC")
	...
	@Query("SELECT COUNT(l) FROM FileAccessLogEntity l WHERE l.fileId = :fileId AND l" +
			".accessType = 'DOWNLOAD'")
	long countDownloadsByFileId(@Param("fileId") UUID fileId);

	@Query("SELECT l FROM FileAccessLogEntity l WHERE l.accessedAt BETWEEN :startTime " +
			"AND :endTime ORDER BY l.accessedAt DESC")
	Page<FileAccessLogEntity> findByAccessedAtBetween(
