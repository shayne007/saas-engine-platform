...
public class InMemoryFileStorageService implements FileStorageService {
	...
	private final Map<String, Map<Integer, String>> uploadChunks =
	...
	public FileQueryResponse queryFiles(FileQueryRequest request) {
		...
		List<FileMetadata> filtered = all.stream()
				.filter(f -> request.getProjectId() == null ||
						Objects.equals(request.getProjectId(), f.getProjectId()))
				.filter(f -> request.getCreatedBy() == null ||
						Objects.equals(request.getCreatedBy(), f.getCreatedBy()))
				.filter(f -> request.getMimeTypes() == null ||
						request.getMimeTypes().isEmpty() ||
						request.getMimeTypes().contains(f.getMimeType()))
