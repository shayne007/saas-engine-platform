...
public class FileStorageServiceImpl implements FileStorageService {
    ...
    public FileUploadResponse uploadFile(FileUploadRequest request) {
        log.info("Processing file upload request for file: {}", request.getOriginalFilename());
        ...
        Optional<FileEntity> existingFile = fileRepository
                .findByFileHashAndProjectId(request.getFileHash(), 
                    request.getProjectId() != null ? UUID.fromString(request.getProjectId()) : null);
        
        if (existingFile.isPresent() && request.getAllowDeduplication()) {
            log.info("File deduplication: returning existing file {}", existingFile.get().getId());
        ...
        try {
            ...
            // Log access
            logFileAccess(fileEntity.getId(), request.getUserId(), FileAccessLogEntity.AccessType.UPLOAD);
            ...
            log.error("Failed to generate upload URL for file {}", fileEntity.getId(), e);
    ...
    public ChunkedUploadResponse initiateChunkedUpload(ChunkedUploadRequest request) {
        log.info("Processing chunked upload request for file: {}", request.getOriginalFilename());
        ...
        try {
            ...
            log.error("Failed to initiate chunked upload for file {}", fileEntity.getId(), e);
            fileEntity.setUploadStatus(FileEntity.UploadStatus.FAILED);
            fileRepository.save(fileEntity);
    ...
    public FileDownloadResponse getDownloadUrl(String fileId, Duration expiration) {
        ...
        try {
            ...
            // Log access
            logFileAccess(fileEntity.getId(), "system", FileAccessLogEntity.AccessType.DOWNLOAD);
    ...
    public FileQueryResponse queryFiles(FileQueryRequest request) {
        log.debug("Querying files with filters: projectId={}, createdBy={}, mimeTypes={}", 
        ...
        UUID projectId = request.getProjectId() != null ? UUID.fromString(request.getProjectId()) : null;
        UUID createdBy = request.getCreatedBy() != null ? UUID.fromString(request.getCreatedBy()) : null;
    ...
    public void deleteFile(String fileId, String userId) {
        ...
        try {
            // Delete from GCS
            boolean deleted = gcsClient.deleteBlob(fileEntity.getGcsBucket(), fileEntity.getGcsObjectKey());
            if (!deleted) {
            ...
            // Log access
            logFileAccess(fileEntity.getId(), userId, FileAccessLogEntity.AccessType.DELETE);
    ...
    private void validateUploadRequest(FileUploadRequest request) {
        ...
        if (request.getFileHash() == null || request.getFileHash().length() != 64) {
            throw new IllegalArgumentException("File hash must be 64 characters (SHA-256)");
        }
    ...
    private void validateChunkUploadRequest(ChunkUploadRequest request, ChunkedUploadEntity chunkedUpload) {
    ...
    private FileEntity createFileEntity(FileUploadRequest request) {
        ...
        return FileEntity.builder()
                ...
                .projectId(request.getProjectId() != null ? UUID.fromString(request.getProjectId()) : null)
    ...
    private String generateObjectKey(FileUploadRequest request) {
        ...
        return String.format("files/%s/%s/%s/%s", userId, projectId, timestamp, generateUniqueFilename(request.getOriginalFilename()));
    }
    ...
    private FileMetadata convertToFileMetadata(FileEntity fileEntity) {
        FileMetadata metadata = FileMetadata.builder()
            ...
            .projectId(fileEntity.getProjectId() != null ? fileEntity.getProjectId().toString() : null)
    ...
    private void logFileAccess(UUID fileId, String userId, FileAccessLogEntity.AccessType accessType) {
        try {
            ...
            log.warn("Failed to log file access for file {} by user {}", fileId, userId, e);
        }
    }
}
